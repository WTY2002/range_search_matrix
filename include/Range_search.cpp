 ...
 * @return 状态码，1：成功；0：失败
 */
int range_search(char* fileString, char* resultFilePath) {
    // 读取查询请求
    ...
    // 保存下边界
    vector<double> lower_bound(query_data.size() + 4);
    // 保存上边界
    vector<double> upper_bound(query_data.size() + 4);
    for (int i = 0; i < query_data.size(); i++) {
        lower_bound[i] = query_data[i][0] * r21; // 下边界
        upper_bound[i] = query_data[i][1] * r21; // 上边界
    }
    lower_bound[query_data.size()] = r21;
    upper_bound[query_data.size()] = r21;
    lower_bound[query_data.size() + 1] = r22 * r21;
    upper_bound[query_data.size() + 1] = r22 * r21;
    lower_bound[query_data.size() + 2] = r22 * r21;
    upper_bound[query_data.size() + 2] = r22 * r21;
    lower_bound[query_data.size() + 3] = -r23;
    upper_bound[query_data.size() + 3] = -r23;
    ...
    for (int i = 0; i < query_data.size(); i++) { // 对每个查询范围进行加密
        vector<double> t(6); // 创建一个大小为6的向量t
        t[0] = r21;
...
EXPORT_SYMBOL int query_algo(char* fileString, char* resultFilePath) {
    return range_search(fileString, resultFilePath);
}
